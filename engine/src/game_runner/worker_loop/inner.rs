//! Per-worker hot loop body (cycle 3 Wave 10 Batch A).
//!
...
//!
//! The hot-loop lint suppression migrates here from pre-split
//! `worker_loop.rs:260` per U10 / J.2.a binding decision (Wave 10 PREP §M.4):
//! sub-fn extraction of `run_worker_thread` is **explicitly out of Wave 10
//! scope**; the suppression count stays at the Wave 9 close baseline.
...
///
/// The hot-loop lint suppression below migrates from pre-split
/// `worker_loop.rs:260` per Wave 10 U10/J.2.a binding decision. Sub-fn
/// extraction of this body is deferred to a future opportunistic wave.
#[allow(clippy::too_many_lines)] // hot loop body migrated from pre-split worker_loop.rs:260; sub-fn extraction deferred per Wave 10 U10/J.2.a.
pub(super) fn run_worker_thread(
    ...
    // §P51: `sym_tables` is `&'static SymTables` (Copy); alias for readability.
