...
fn opening_plies_are_not_pushed_to_buffer() {
    ...
    while completed_workers.len() < 2 && attempts < 100 {
        for (_plies, _winner_code, _move_history, worker_id, _tr, _vmin, _vmax, _vd, _seeded, _sf) in runner.drain_game_results() {
...
fn opening_move_is_in_the_legal_set() {
    ...
    while !saw_any_game && attempts < 100 {
        for (plies, _winner_code, move_history, _worker_id, _tr, _vmin, _vmax, _vd, _seeded, _sf) in runner.drain_game_results() {
