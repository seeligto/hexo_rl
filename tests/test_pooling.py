...
def test_minmax_shape_parity_across_K():
    ...
    for K in (1, 2, 4, 6):
        ...
        log_p, val = pool(toks, logp_K, val_K)
...
def test_pma_k1_is_well_defined_and_deterministic():
    ...
    log_p1, val1 = pool(toks, logp_K, val_K)
    log_p2, val2 = pool(toks, logp_K, val_K)
...
def test_pma_invariant_to_duplicate_clusters():
    ...
    out1_lp, out1_v = pool(x, logp, val)
    out2_lp, out2_v = pool(x.repeat(1, 2, 1), logp.repeat(1, 2, 1), val.repeat(1, 2, 1))
