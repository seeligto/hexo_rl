...
def test_rust_runner_with_inference_server_generates_positions():
    ...
    runner = SelfPlayRunner(SelfPlayRunnerConfig(n_workers=2, max_moves_per_game=16, n_simulations=1, leaf_batch_size=1, encoding_name="v6"))
...
def test_rust_runner_collect_data_format():
    ...
    runner = SelfPlayRunner(SelfPlayRunnerConfig(n_workers=1, max_moves_per_game=4, n_simulations=1, leaf_batch_size=1, encoding_name="v6"))
    server = InferenceServer(
        model,
        device,
    ...
    try:
        ...
        # drain_game_results returns metadata 10-tuples (Phase B' instrumentation:
        # terminal_reason + model_version_min/max/distinct; D-WS3V3: seeded +
        # solver_fires). Spatial aux flows per-row via collect_data() instead.
        ...
        # D-WS3V3: default (no seeding / solver OFF) → seeded 0, no fires.
        assert seeded == 0
        assert solver_fires == 0

        # collect_data returns 10 numpy arrays:
        # (feats, chain, pols, vals, plies, own, wl, is_full_search, position_index, value_valid)
        feats_np, chain_np, pols_np, vals_np, plies_np, own_np, wl_np, ifs_np, pidx_np, _vv = runner.collect_data()
