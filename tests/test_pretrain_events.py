...
def test_pretrain_emits_training_step_event():
    model = HexTacToeNet(board_size=19, in_channels=8, filters=16, res_blocks=2, se_reduction_ratio=4)
    config = {
        "lr": 0.01,
    ...
    # dummy loader — 4-tuple: (states, chain_planes, policies, outcomes)
    states = torch.zeros(2, 8, 19, 19, dtype=torch.float32)
