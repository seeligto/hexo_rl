...
def test_sample_returns_correct_shapes():
    ...
    push_n(buf, 100)
    s, c, p, o, own, wl, _ifs, _vv = buf.sample_batch(16, augment=True)
...
def test_sample_no_augment_content_roundtrip():
    ...
    _, _, _, outcomes, _, _, _, _vv = buf.sample_batch(200, augment=False)
...
def test_identity_symmetry_preserves_data():
    ...
    sampled_s, _c, sampled_p, sampled_o, _, _, _, _vv = buf.sample_batch(1, augment=False)
...
def test_pass_action_invariant_under_augmentation():
    ...
    for _ in range(10):
        ...
        _, _c, policies, _, _, _, _, _vv = buf.sample_batch(32, augment=True)
...
def test_policy_sum_preserved_under_augmentation():
    ...
    _, _c, policies, _, _, _, _, _vv = buf.sample_batch(30, augment=True)
...
def test_correlation_guard_no_duplicate_game_ids():
    ...
    for _ in range(20):
        _, _, _, _, _, _, _, _vv = buf.sample_batch(64, augment=False)
...
def test_resize_basic():
    ...
    s, _c, p, o, _, _, _, _vv = buf.sample_batch(5, augment=False)
    assert s.shape[0] == 5
...
def test_resize_preserves_data_after_wrap():
    ...
    s, _c, p, o, _, _, _, _vv = buf.sample_batch(10, augment=False)
...
def test_resize_then_push():
    ...
    s, _c, p, o, _, _, _, _vv = buf.sample_batch(8, augment=False)
    assert s.shape[0] == 8
...
def test_resize_content_roundtrip():
    ...
    seen = set()
    for _ in range(50):
        _, _, _, outcomes, _, _, _, _vv = buf.sample_batch(5, augment=False)
        for o in outcomes:
...
def test_buffer_save_load_roundtrip(tmp_path):
    ...
    states, _c, policies, outcomes, _, _, _, _vv = buf2.sample_batch(10, augment=False)
...
def test_buffer_load_size_mismatch(tmp_path):
    ...
    states, _c, policies, outcomes, _, _, _, _vv = buf_small.sample_batch(10, augment=False)
    assert states.shape == (10, CHANNELS, BOARD_SIZE, BOARD_SIZE)
...
def test_push_many_basic():
    ...
    s, c, p, o, own, wl, ifs, _vv = buf.sample_batch(32, augment=False)
...
def test_push_many_mixed_is_full_search():
    ...
    _, _, _, _, _, _, ifs, _vv = buf.sample_batch(n, augment=False)
    assert ifs.shape == (n,)
