...
**Human game constraint:** [site-redacted] API limit is 500 games per pull.
...
confirmed it is the source code for [site-redacted] — the site we already
...
1. **Scraper enhancements:**
   ...
   - Updated `UNAUTH_LIMIT` from 480 to 500 (confirmed from
...
**Official rule:** [site-redacted] states "a new hex can be placed at most 8 cells apart
...
### §65 — Dashboard tracer: policy_excess formula fix + mcts_mean_depth confirmed (2026-04-07)

**Trigger:** Live dashboard at step ~3,800 showed mcts_mean_depth = 2.4 and
policy_excess = −0.77. Both appeared anomalous and were investigated.

**Finding 1 — branch confusion:** The §64 diagnostic checked master (HEAD
e15a18f) while the live run was on branch phase4.5/monitoring-ratios-mcts-health.
Both MCTS metrics are fully implemented on the feature branch. No missing
implementation.

**Finding 2 — mcts_mean_depth = 2.4 is correct:** Depth is accumulated
per-sim in select_leaves() outer loop (not inside select_one_leaf traversal),
reset per-game in new_game(). At step 3,800 with near-uniform priors, wide
ZOI, and 25% fast games (50 sims), mean leaf depth of 2–4 is physically
correct. Not a bug. Monitoring threshold: expect ≥ 5.0 by step 50–70k
(adjusted for decay_steps=300k — network transitions more slowly than the
original ≥5.0@20k estimate assumed).

**Finding 3 — truncation bug self-corrected:** First commit (46b1e5a)
accumulated depth as u64 (truncating the f32 per-game mean). Fixed in
e3ef62d (×1M fixed-point scaling) committed 38 minutes later. No action
needed.

**Finding 4 — policy_excess formula is wrong:** policy_excess was designed
when loss_policy = CE. After §61 switched to KL, the formula became
KL − H(target) = CE − 2·H(target) which has no guaranteed sign and is
negative during normal learning. Resolution: Option A — rename to "policy KL"
and display loss_policy directly (it IS KL after §61). Implemented in §65
follow-up commit.

**Config note:** decay_steps=300k confirmed correct for both laptop and
desktop runs. 50k would be too aggressive (70% self-play before step 50k,
before network is reliable). Do not change.