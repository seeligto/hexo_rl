...
| Worker throughput | 177,799 pos/hr | ≥ 142,000 pos/hr | IQR ±143,850 (**80.9% — bimodal**); range [0–198.5k]. One of the 5 runs completed 0 games — same startup-race pattern §123/§102 fought. Median is robust; if mean trips downstream alerts, raise n_runs or pool_duration. **Trace path matches compile-on within noise band on this hardware** (171k §123 vs 178k now). On dispatch-bound hardware (EPYC 4080S) trace is expected to lift more — sweep validation pending |
