...
**FINAL — all three cheap levers exhausted; restart NOT justified.** value-target distillation (E1, KILL-C 0.441) ∧ light-trunk finetune (LT, 0.532) ∧ richer threat input features (E2, 0.155 / ceiling AUC 0.646) ALL ENTANGLE. The deep-forced-loss value blind-spot is robust to output-distillation, trunk-finetune, AND cheap input features = a DEEP/HORIZON limit (consistent with D-PERCEPT 67% deep value-blind / ~15% short-lookahead-catchable, and §D-LADDER P3 search-scaling PLATEAU-by-150). The `v6_live2_ft` 8-plane bootstrap restart is NOT evidence-justified — it would burn GPU-weeks on a representation whose held-out ceiling is already measured low. Remaining directions are expensive/architectural (deeper net / search-in-the-loop / accept-the-limit) or a cheap deeper-search-distilled value-TARGET discriminator (likely also entangles — bottleneck is representational — but cheap to falsify). Do NOT relabel more wins (in-sample 46% recall ruled win-count out). Do NOT launch any GPU-week without a cheap discriminator first clearing KILL-C≥0.85 against a valid entangled control.
