...
**Path → bootstrap restart now EVIDENCE-JUSTIFIED from BOTH ends (frozen + unfrozen entangle).** The fix is representational, not a target trick. **Next gate = E2 threat-plane re-probe** (`scripts/dvderisk_e2_reprobe.py`, ~30 min, no restart yet) — do the richer INPUT features separate? E2 spec (`reports/d_fullspec_2026-06-26/E2_feature_spec.md`): 4 opponent-agnostic threat planes from existing `engine/src/board/threats.rs:check_window` — `cur/opp_open_line_count` + `cur/opp_best_window_fill` (completion proximity). Empirical: `opp_best_window_fill ≥ 4/6` (opponent forced threat) in 30.1% loss vs 8.8% win positions (3.4×; spatial peak concentration, means within 0.03). n_planes 4→8 ⇒ encoding `v6_live2_ft`, fresh pretrain (not surgically patchable). Do NOT relabel more wins (in-sample 46% recall ruled win-count out). Do NOT add the TOML entry / launch the restart until the E2 re-probe verdict is in hand.

### Falsified Hypotheses Register addition (§D-FULLSPEC 2026-06-26)
- **"The value blind-spot is a TARGET problem — loss-only distillation just lacked the win-contrast gradient; full-spectrum class-balanced distillation on existing v6_live2 features will SEPARATE wins from losses (KILL-A>0.35 AND KILL-C≥0.85 together)."** → **FALSIFIED.** Frozen-trunk full-spectrum distill on turn-phase-matched held-out: KILL-A 0.76 ✅ but KILL-C 0.44 ❌ (six estimates crater [0.32–0.46]); in-sample train-fit 46% rules out under-power; D-INJECT anti-correlation persists. The existing 4-plane features CONFLATE win/loss in the net_value>0 blind-spot neighborhood once the turn-phase shortcut (AUC 0.807) is controlled. ⇒ FEATURE problem, not TARGET. **Re-validation closed:** the registered light-trunk-unfreeze precursor RAN (2026-06-26) = ENTANGLED_LT — trunk adaptation also fails to separate (best 0.653/0.532, joint_pass=0, raises KILL-A only by globally depressing value). So a light-trunk finetune target fix is ALSO falsified; the FEATURE diagnosis holds from both frozen and unfrozen ends. Remaining open: whether richer INPUT features (E2 threat planes) separate — gated on `dvderisk_e2_reprobe.py`.
