...
**P0/P1/P3:** P0 — depth-5 = reproducible SealBot bar; **unit = HTTT TURNS** (depth-5 ≈ 10 stones; 5 = median+1). P1 — banked jsonl PARTIAL (moves, no value) → re-eval, no fresh games. **P3 (search-scaling 150-vs-256) LAUNCHED on vast** (tmux `p3sweep`, co-tenant nice-19 with the live d1m run; `scripts/run_d_localize_p3_vast.sh` + `--n-sims-override` patch); PENDING; runbook `docs/handoffs/d_localize_p3_search_scaling_vast_runbook.md`.
