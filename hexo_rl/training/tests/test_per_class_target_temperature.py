...
def test_resolve_config_resolves_non_unit_temperatures():
    ...
    assert res == {
        ...
        "apply_to_selfplay": True,  # default backward-compat
        "selfplay_sample_rate": 1.0,
    }


def test_resolve_config_default_apply_to_selfplay_is_true():
    """Wave 2 backward compat — default scope is selfplay-only."""
    cfg = {"per_class_target_temperature": {
        "enabled": True, "colony_temperature": 1.5,
    }}
    res = _resolve_config(cfg)
    assert res["apply_to_selfplay"] is True
    assert res["apply_to_pretrain"] is False


def test_resolve_config_both_scopes_off_returns_none():
    """L52 Wave 3 guard — explicit no-op when neither slice is in scope."""
    cfg = {"per_class_target_temperature": {
        "enabled": True, "colony_temperature": 1.5,
        "apply_to_pretrain": False, "apply_to_selfplay": False,
    }}
    assert _resolve_config(cfg) is None
...
# ── Wave 3 scope revision (L52) ─────────────────────────────────────────────
def test_apply_to_selfplay_false_skips_selfplay_rows():
    """L52 Wave 3 scope — selfplay rows untouched when apply_to_selfplay=false."""
    # Batch: [pretrain (2 colony, untouched even with default scope),
    #         selfplay (2 colony, would be softened under default)]
    states = torch.from_numpy(np.stack([
        _colony_state(), _colony_state(),       # pretrain
        _colony_state(), _colony_state(),       # selfplay
    ])).float()
    policies = _peaked_policy(4)
    initial = policies.clone()
    ...
    out = apply_per_class_temperature(
        policies, states, n_pretrain=2,
        config=cfg, device=torch.device("cpu"),
    )
    # Both scopes off → no-op → input returned unchanged.
    assert out is policies, (
        "Both apply_to_pretrain=false + apply_to_selfplay=false must short-circuit "
        "to a no-op (return input object unchanged)."
    )
    assert torch.equal(out, initial)


def test_apply_to_selfplay_false_with_pretrain_true_pretrain_only():
    """L52 Wave 3 main scope — only pretrain rows get the temperature scaling."""
    states = torch.from_numpy(np.stack([
        _colony_state(), _colony_state(),       # pretrain — should be softened
        _colony_state(), _colony_state(),       # selfplay — should NOT be touched
    ])).float()
    policies = _peaked_policy(4)
    initial = policies.clone()
    ...
    # Pretrain rows softened — peaks drop below 0.90 initial.
    assert out[0, 0].item() < 0.90
    assert out[1, 0].item() < 0.90
    # Selfplay rows untouched — bit-identical to initial.
    assert torch.allclose(out[2], initial[2])
    assert torch.allclose(out[3], initial[3])
    # All rows still sum to 1.0.
    sums = out.sum(dim=1)
    assert torch.allclose(sums, torch.ones(4), atol=1e-5)


def test_apply_to_pretrain_true_with_selfplay_true_targets_all_rows():
    """Both scopes true → behaves like the existing apply_to_pretrain path
    (all rows in scope). Regression guard — apply_to_selfplay default
    preserves the union semantics."""
    states = torch.from_numpy(np.stack([
        _colony_state(), _colony_state(),       # pretrain
        _colony_state(), _colony_state(),       # selfplay
    ])).float()
    ...
    # All four rows softened — same behaviour as the existing
    # apply_to_pretrain=True path (which now implicitly relies on
    # apply_to_selfplay=True default).
    for i in range(4):
        assert out[i, 0].item() < initial[i, 0].item(), (
            f"row {i} colony peak should be softened"
        )


def test_subsample_rate_does_not_affect_pretrain_rows():
    """Sub-sampling applies to selfplay rows only — pretrain always fully classified."""
    torch.manual_seed(0)
    # 4 pretrain (all colony) + 20 selfplay (all colony).
    states = torch.from_numpy(np.stack([_colony_state() for _ in range(24)])).float()
    policies = _peaked_policy(24)
    initial = policies.clone()
    ...
    cfg = {"per_class_target_temperature": {
        ...
        "apply_to_pretrain": True,
        "apply_to_selfplay": True,
        "selfplay_sample_rate": 0.25,  # 25% of 20 = 5 selfplay rows
    }}
    out = apply_per_class_temperature(
        policies, states, n_pretrain=4,
        config=cfg, device=torch.device("cpu"),
    )
    # All 4 pretrain rows touched regardless of sample_rate.
    for i in range(4):
        assert out[i, 0].item() < initial[i, 0].item(), (
            f"pretrain row {i} should be softened independent of selfplay sample rate"
        )
    # Selfplay rows: 5 sampled (peaks changed), 15 unsampled (unchanged).
    sp_changed = sum(
        bool((out[i, 0] != initial[i, 0]).item()) for i in range(4, 24)
    )
    assert sp_changed == 5
